...
class simple_faucet
	...
	public function __construct($config)
		...
		$defaults = array(
			...
			"donation_address" => "DNd7tFAMpnmv1UpYV9bdpjdhXwQZbEmDC2",
			"title" => "DOGE Faucet",
			"template" => "default",
						...
						if (isset($_POST["dogecoin_address"]) && (($this->config["use_captcha"] && $this->valid_captcha()) || !$this->config["use_captcha"]))
							...
							if ($validation["isvalid"])
								...
								if ($row = $result->fetch_assoc())
									...
									$this->db->query("INSERT INTO `".$this->db->escape_string($this->config["mysql_table_prefix"])."payouts` (`payout_amount`,`ip_address`,`payout_address`,`timestamp`) VALUES ('".$this->payout_amount."','".$this->db->escape_string($_SERVER["REMOTE_ADDR"])."','".$this->db->escape_string($dogecoin_address)."',NOW())"); // insert the transaction into the payout log
									if ($this->config["wallet_passphrase"] != "")
										$this->rpc("walletpassphrase",array($this->config["wallet_passphrase"],4)); // unlock wallet
									$this->status = !is_null($this->rpc("sendtoaddress",array($dogecoin_address,$this->payout_amount))) ? SF_STATUS_PAYOUT_ACCEPTED : SF_STATUS_PAYOUT_ERROR; // send the DOGE
									if ($this->config["wallet_passphrase"] != "")
	...
	public function render()
		...
		$template = preg_replace_callback("/\{\{([a-zA-Z-0-9\ \_]+?)\}\}/",function($match) use ($self,$config,$balance,$payout_amount,$payout_address)
			{
